...
function DebtorsContent() {
    ...
    useEffect(() => {
        fetch('/api/qbo/reports/aged-receivables')
            .then(res => res.json())
            .then(data => {
                setReportData(data);
                setLoading(false);
    ...
    return (
        <div className="space-y-6">
            ...
            <Card className="glass border-white/10">
                <CardHeader>
                    <CardTitle className="text-foreground">Outstanding Receivables</CardTitle>
                </CardHeader>
                <CardContent>
                        ...
                        <table className="min-w-full divide-y divide-white/10">
                            ...
                            <tbody className="bg-transparent divide-y divide-white/10">
                                {rows && rows.map((row: any, i: number) => {
                                    // Handle Summary Rows or Data Rows. 
                                    // QBO logic: row.ColData contains values. If group, it has 'Rows'.
                                    // Simplification: We iterate top-level.
